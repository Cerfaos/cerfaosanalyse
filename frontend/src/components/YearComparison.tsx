...
export default function YearComparison() {
  const [metric, setMetric] = useState<'distance' | 'duration' | 'trimp' | 'count'>('distance')
  const [currentYearData, setCurrentYearData] = useState<MonthData[]>([])
  const [yearStats, setYearStats] = useState<{ current: YearStats; previous: YearStats } | null>(null)
  ...
  const calculateMetric = (activities: any[], metricType: string): number => {
    switch (metricType) {
      case 'distance':
        return Math.round(activities.reduce((sum, a) => sum + (a.distance || 0), 0) / 1000)
      case 'duration':
        return Math.round(activities.reduce((sum, a) => sum + (a.duration || 0), 0) / 3600)
      case 'trimp':
        return Math.round(activities.reduce((sum, a) => sum + (a.trimp || 0), 0))
      case 'count':
  ...
  if (loading) {
    return <div className="text-center py-8 text-text-muted">Chargement de la comparaison...</div>
  }
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="glass-panel p-4 text-center">
            ...
            <div className="text-xl font-bold">
              ...
              <span className="text-sm text-text-muted ml-1">vs {yearStats.previous.totalActivities}</span>
            </div>
            <div
            ...
            >
              {formatDifference(yearStats.current.totalActivities, yearStats.previous.totalActivities)}
            </div>
          </div>
          ...
          <div className="glass-panel p-4 text-center">
            ...
            >
              {formatDifference(yearStats.current.totalDistance, yearStats.previous.totalDistance)}
            </div>
          </div>
          ...
          <div className="glass-panel p-4 text-center">
            ...
            >
              {formatDifference(yearStats.current.totalDuration, yearStats.previous.totalDuration)}
            </div>
          </div>

          <div className="glass-panel p-4 text-center">
            <div className="text-sm text-text-muted mb-1">TRIMP total</div>
            <div className="text-xl font-bold">{Math.round(yearStats.current.totalTrimp)}</div>
            <div
              className={`text-sm font-medium ${
                yearStats.current.totalTrimp >= yearStats.previous.totalTrimp ? 'text-green-600' : 'text-red-500'
              }`}
            >
              {formatDifference(yearStats.current.totalTrimp, yearStats.previous.totalTrimp)}
            </div>
          </div>
        </div>
      )}
          ...
          <BarChart data={currentYearData} barGap={2}>
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => [
                `${value} ${metric === 'distance' ? 'km' : metric === 'duration' ? 'h' : metric === 'trimp' ? 'pts' : ''}`,
            ...
            />
            <Bar dataKey="previousYear" fill="#9CA3AF" name="previousYear" radius={[4, 4, 0, 0]} />
            <Bar dataKey="currentYear" fill="#059669" name="currentYear" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
      </div>

      <div className="text-center text-sm text-text-muted">{getMetricLabel()} par mois</div>
    </div>
  )
}
